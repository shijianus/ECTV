...
async function getInitConfig(
  ...
  const adminConfig: AdminConfig = {
    ...
    SiteConfig: {
      SiteName: process.env.NEXT_PUBLIC_SITE_NAME || 'ECTV',
      Announcement:
...
export function configSelfCheck(adminConfig: AdminConfig): AdminConfig {
  ...
  if (!adminConfig.SiteConfig) {
    adminConfig.SiteConfig = {
      SiteName: 'ECTV',
      Announcement: '免责声明：本网站仅作为影视信息聚合工具，所有视频内容均由互联网第三方接口自动抓取或由用户自行配置，本站不存储、不制作任何视频资源。用户使用本站即代表同意自行承担因观看、传播相关内容而可能引起的任何法律风险。本站不对内容的准确性、合法性负责，如有侵权请联系第三方数据源删除。',

...
- **中立工具性质**：本项目仅作为技术研究与个人学习使用的播放器工具，不提供、不存储、不分发任何音视频资源。
- **用户责任自担**：用户在软件中导入或添加的任何第三方内容源（如播放源、直播源、弹幕源等）均属于用户个人行为。用户必须确保所使用的资源具有合法授权。
- **法律风险承担**：因用户非法使用、传播受版权保护的内容而产生的任何法律责任，均由用户本人独立承担。项目开发者及站点部署者不对此承担任何直接、间接、连带或刑事责任。
- **版权方须知**：如您发现该实例被用户用于展示侵权内容，请立即通过以下方式联系站点负责人进行阻断处理：
  - **Telegram 机器人**：[t.me/epocanvas_bot](https://t.me/epocanvas_bot)
  - **联系邮箱**：[admin@epocanvas.com](mailto:admin@epocanvas.com)
  站点负责人将在核实后第一时间清理相关配置。
- **地域法律合规**：本项目不在中国大陆地区提供服务。在该地区使用所产生的法律风险及责任，属于用户个人行为，须自行承担全部责任。特此声明。
